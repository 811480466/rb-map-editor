...
(function wildPanelModule() {
  ...
  function normalizePokemonDataIfNeeded() {
    if (typeof window.RBEditorNormalizePokemonData === "function") {
      window.RBEditorNormalizePokemonData();
    }
  }
  ...
  function formatPokemonHtml(speciesId) {
    const id = Number(speciesId);
    const item = getPokemonById(id);
  ...
  function buildPokemonOptions() {
    return getPokemonList().map(item => {
      const id = Number(item.id);
      if (!Number.isInteger(id)) return "";
      const name = item.name || item.code || `#${id}`;
      const code = item.code && item.code !== name ? ` / ${item.code}` : "";
      return `<option value="${escapeText(`${name} #${id}${code}`)}"></option>`;
    }).join("");
  }
  ...
  function renderEntriesTable(group) {
    ...
    const datalistId = `wildPokemonList_${group.kind}`;
    const rows = group.entries.map(entry => `
      <tr data-entry-offset="${entry.offset}">
        ...
        <td>
          <input class="wild-pokemon-input" type="text" list="${datalistId}" data-wild-field="species" data-original-species="${entry.species}" value="${escapeText(formatPokemonText(entry.species))}">
        </td>
    ...
    return `
      <div class="wild-table-wrap">
        ...
        <datalist id="${datalistId}">${buildPokemonOptions()}</datalist>
    ...
    applyBtn.onclick = () => {
      try {
        ...
        for (const row of rows) {
          ...
          const speciesInput = row.querySelector('[data-wild-field="species"]');
          const species = parsePokemonInput(speciesInput?.value);
          if (species === null) {
            throw new Error(`无法识别宝可梦：${speciesInput?.value || "空"}`);
          }
